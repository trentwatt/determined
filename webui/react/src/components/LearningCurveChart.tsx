...
const LearningCurveChart: React.FC<Props> = ({
  ...
  const singleBatch = chartData[0]?.length === 1 ? chartData[0][0] : undefined;
  ...
  const chartOptions: Options = useMemo(() => {
    ...
    return {
      ...
      scales: { x: singleBatch ? { max: 2 * singleBatch, min: 0, time: false } : { time: false } },
      series: [
        { label: 'batches' },
  ...
  }, [ onTrialClick, onTrialFocus, selectedMetric, trialIds, singleBatch ]);
